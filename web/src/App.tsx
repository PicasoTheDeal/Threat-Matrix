...
const TURNSTILE_SITE_KEY = "0x4AAAAAADyqlI0EhIwOLfsF";
...
export default function App() {
  ...
  const fetchUserParameters = async (authToken: string) => {
    try {
      const res = await fetch("http://localhost:8787/api/parameters", {
  ...
  const syncUserParameters = async (newTags: string[]) => {
    if (!token) return;
    try {
      await fetch("http://localhost:8787/api/parameters", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    const fetchAndMerge = async () => {
      try {
        ...
        const res = await fetch("http://localhost:8787/api/logs", { headers });
    ...
    const fetchBatch = async () => {
      for (let i = 0; i < logIds.length; i += batchSize) {
        ...
        try {
          ...
          const res = await fetch(`http://localhost:8787/api/interact?log_ids=${encodeURIComponent(idsParam)}`, { headers });
          if (res.ok) {
            const data = await res.json();
  ...
  const handleLikeToggle = async (logId: string) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8787/api/interact", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
  ...
  return (
    ...
    >
      {/* Hidden container for invisible Turnstile widget */}
